...
// --- A MÁGICA DE LEITURA UNIVERSAL (JSON + URL ENCODED) ---
...
// O motor principal de extração de dados
async function processAsaasWebhook(payload) {
  ...
  // 1. Prioridade: Buscar pelo External Reference (ID do Usuário)
  ...
  // 2. Se não achou por ID, usar API para pegar E-mail
      ...
      if (res.ok) {
        ...
        if (phonePagador) {
          ...
          if (user) {
            ...
            return { ok: true, reason: 'success_by_phone', email: user.email };
          }
        }
      }
    } catch (err) {
...
// ==========================================
// 💸 WEBHOOK ASAAS (VERSÃO DEFINITIVA 4.0)
// ==========================================
app.post(['/api/webhook/asaas', '/api/webhooks/asaas'], async (req, res) => {
  try {
    let payload = req.body;

    console.log("📥 [ASAAS] Headers Content-Type:", req.headers['content-type']);

    // Desempacotador Asaas
    ...
    if (!payload || typeof payload !== 'object' || !payload.event) {
      console.log('⚠️ [ASAAS] Payload final vazio/inválido.');

...
// --- COMPONENTES AUXILIARES (FORA PARA EVITAR ERROS) ---
...
export default function App() {
  ...
  const handleUpdatePassword = async () => {
    ...
    if (res.ok) { showToast("Senha alterada!"); setShowPasswordModal(false); } else showToast("Erro na senha.");
  };
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6">
       <div className="w-full max-w-md bg-slate-900 border border-slate-800 p-10 rounded-[3rem] shadow-2xl text-center">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button className="w-full p-6 bg-blue-600 rounded-3xl font-black text-white uppercase active:scale-95 transition-all shadow-xl">{isLoggingIn ? 'Entrando...' : 'Acessar Painel'}</button>
          </form>
       </div>
    </div>
  );
      ...
      <main className="flex-1 p-6 md:p-10 max-w-7xl mx-auto w-full pb-40 animate-fade-in">
        ...
        {tabAtiva === 'dashboard' && (
          <div className="space-y-10">
             <h2 className="text-4xl font-black italic tracking-tight uppercase">{isMaster ? 'Analytics' : 'Coach Dash'}</h2>
             <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <StatCard title="Saúde" value="98%" icon={Target} color="bg-amber-500" />
             </div>
          </div>
        )}
        ...
        {tabAtiva === 'alunos' && (
          <div className="space-y-10">
             <div className="flex justify-between items-center">
                <h2 className="text-3xl font-black italic uppercase">{isMaster ? 'Treinadores' : 'Sua Tropa'}</h2>
                  ...
                  <div key={item.id} className="bg-slate-900 border border-slate-800 p-8 rounded-[3.5rem] shadow-xl hover:border-slate-700 transition-all flex flex-col gap-6">
                     ...
                     <div className="flex gap-2">
                        <button onClick={() => { 
                          ...
                          {isMaster ? 'Planos' : 'Gerenciar'}
                        </button>
                        {item.phone && <button onClick={() => openWhatsApp(item.phone, item.name)} className="p-5 bg-emerald-600/10 text-emerald-500 rounded-2xl shadow-xl"><MessageCircle size={22}/></button>}
        ...
        {tabAtiva === 'ia' && (
           <div className="max-w-3xl mx-auto space-y-10 text-center">
              <div className="bg-gradient-to-br from-indigo-700 to-blue-900 p-12 rounded-[4rem] shadow-2xl">
                 ...
                 <h2 className="text-4xl font-black italic text-white tracking-tighter uppercase">EvoIntelligence™</h2>
                 <p className="text-blue-100 mt-4 opacity-80 leading-relaxed italic">"Dite os comandos biomecânicos no Claude Desktop para o aluno selecionado."</p>
              </div>
           </div>
        )}
        ...
        {tabAtiva === 'perfil' && (
          <div className="max-w-2xl mx-auto space-y-10 text-center animate-fade-in pb-40">
             <h2 className="text-3xl font-black italic text-center uppercase tracking-tight">Gerenciar <span className="text-blue-500">Conta</span></h2>
        ...
        <div className="fixed inset-0 bg-black/98 z-[800] flex items-center justify-center p-6 backdrop-blur-md">
           <div className="bg-slate-900 border border-slate-800 p-10 rounded-[3rem] w-full max-w-md shadow-2xl text-center">
              ...
              <div className="space-y-4 text-left">
                 ...
                 <button onClick={handleUpdatePassword} className="w-full py-6 bg-blue-600 text-white font-black rounded-3xl active:scale-95 shadow-xl transition-all uppercase tracking-widest text-[11px] mt-4">Atualizar Agora</button>
      ...
      {showAddAlunoModal && (
        <div className="fixed inset-0 bg-black/98 z-[600] flex items-center justify-center p-6 backdrop-blur-md animate-fade-in">
           <div className="bg-slate-900 border border-slate-800 p-10 rounded-[4rem] w-full max-w-2xl max-h-[90vh] overflow-y-auto custom-scrollbar shadow-2xl">
              <div className="flex justify-between items-center mb-10 pb-6 border-b border-slate-800"><h3 className="text-3xl font-black text-white italic tracking-tighter uppercase">Recrutar Atleta</h3><button onClick={() => setShowAddAlunoModal(false)} className="p-3 bg-slate-800 rounded-2xl text-slate-400"><X size={28}/></button></div>
              <form onSubmit={handleCreateAluno} className="space-y-6">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <input type="text" placeholder="Nome do Atleta" required className="p-6 rounded-3xl bg-slate-950 border border-slate-800 text-white font-bold" value={formAluno.name} onChange={e => setFormAluno({...formAluno, name: e.target.value})} />
                 ...
                 <textarea placeholder="Anamnese / Histórico Médico / Restrições para a IA..." className="w-full p-8 bg-slate-950 border border-slate-800 rounded-[2.5rem] text-white min-h-[150px]" value={formAluno.anamnese} onChange={e => setFormAluno({...formAluno, anamnese: e.target.value})}></textarea>
                 <button type="submit" className="w-full py-7 bg-blue-600 text-white font-black rounded-[2rem] shadow-2xl uppercase tracking-widest text-[12px]">Finalizar e Sincronizar IA</button>
              </form>
           </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/98 z-[600] flex items-center justify-center p-6 backdrop-blur-md animate-fade-in">
           <div className="bg-slate-900 border border-slate-800 p-10 rounded-[4rem] w-full max-w-md shadow-2xl">
              ...
              <form onSubmit={handleCreateTrainer} className="space-y-4">
                 ...
                 <input type="tel" placeholder="WhatsApp" required className="w-full p-5 rounded-2xl bg-slate-950 border border-slate-800 text-white" value={formTrainer.phone} onChange={e => setFormTrainer({...formTrainer, phone: e.target.value})} />
                 ...
                 <button type="button" onClick={() => setShowAddTrainerModal(false)} className="w-full py-2 text-slate-500 font-bold text-xs uppercase">Cancelar</button>
              </form>
           </div>
        </div>
      )}
